...
resource "aws_iam_policy" "sf_ses_email_policy" {
  name        = "SFSendEmailPolicy"
  description = "Allow Step Functions to send emails sf."
...
resource "aws_iam_role_policy_attachment" "sfn_sf_execute_attach" {
  policy_arn = aws_iam_policy.sf_ses_email_policy.arn
  role       = aws_iam_role.step_functions_role.name
}
